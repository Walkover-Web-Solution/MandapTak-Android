...
public class MessageView {
    ...
    private Message.RecipientStatus getMessageStatus(Message msg) {
        ...
        if (!msg.getSender().getUserId().equalsIgnoreCase(ChatsFragment.getUserID()))
        ...
        for (int i = 0; i < ChatsFragment.getAllParticipants().size(); i++) {

            //Don't check the status of the current user
            String participant = ChatsFragment.getAllParticipants().get(i);
            if (participant.equalsIgnoreCase(ChatsFragment.getUserID()))
                continue;
